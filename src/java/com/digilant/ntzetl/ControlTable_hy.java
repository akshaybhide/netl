...
public abstract class ControlTable_hy
{

    protected Integer fallBehind;

    protected abstract List<String> nextBatch(int lookback,ExcName exchange);
    //	protected abstract List<String> nextDay(String oneday);
    // TODO: switch this back to protected
    public abstract void reportFinished(Collection<String> compset);
    private ExcName adexchange;
    protected abstract boolean finishedForDay(String daycode);

    public abstract void printStatusInfo();

    public SortedSet<String> nextBatch(int lookback, int maxfiles,ExcName exchange)
    {
	Comparator<String> simplenamecomp = new Comparator<String>()
	    {
		public int compare(String a, String b)
		{
		    return SliUtil_hy.pathSimpleName(a).compareTo(SliUtil_hy.pathSimpleName(b));
		}
	    };
	adexchange=exchange;

	List<String> fullbatch = nextBatch(lookback,adexchange);
    ...
    public static class CleanListImpl extends ControlTable_hy
    {
	...
	public CleanListImpl(String cdir)
	{
	    ...
	    for(String oneday : getDateRange(SliUtil_hy.CLEANLIST_SAVE_WINDOW))
		{
		    // e.g. 2012-06-15____clean.txt
		    String savepath = SliUtil_hy.getCleanListPath(_cleanDir,oneday);
	...
	{
	    while(_cleanMap.size() > SliUtil_hy.CLEANLIST_SAVE_WINDOW)
	...
	private void persist()
	{
		...
		{
		    String cleanpath = SliUtil_hy.getCleanListPath(_cleanDir,daycode);
	...
	public boolean pathIsClean(String checkpath)
	{
	...
	public List<String> remainList(List<String> pathlist)
	{
	    List<String> newlist = Util.vector();
	    for(String oneday : pathlist)
		{
	...
	public List<String> nextBatch(int lookback, ExcName exchange)
	{
	    List<String> newlist = Util.vector();
	    for(String oneday : getDateRange(lookback))
		{
		    for(String onepath : SliUtil_hy.getPathsForExc(oneday,exchange))
			{
			    if(!pathIsClean(onepath))
				{ newlist.add(onepath); }
			}
	    ...
	    public boolean finishedForDay(String daycode)
	{
	    ...
	    for(String onepath : SliUtil_hy.getPathsForDay(daycode))
		{
		    if(!pathIsClean(onepath))
			{ return false; }
		}
    ...
    public static class DbImpl extends ControlTable_hy
    {
	...
	/*		public List<String> nextDay(String one)
			{
			Vector<String> pathset = new Vector<String>(_batchSet);
			return pathset;
			}*/
	...
	private void loadNewPaths(int numdays, int maxadd)
	{
	    ...
	    for(String oneday : dayrange)
		{
		    ...
		    for(String onepath : SliUtil_hy.getPathsForDay(oneday))
			{
