...
public class TestTiming 
{
	...
	{
		TestTiming ttime = new TestTiming("2012-05-29", 1);
		// ttime.flushToStaging();
		
		// ttime.checkLockMove();
		
	}	
	public TestTiming(String date, int lookback) throws IOException{
	...
	void startUp(String date, int lookback)
	{
		...
		for(int i = 0; i < pathlist.size() ; i++)
		{
			...
			if((i % 10) == 0)
			{
				double now = Util.curtime();
				double avg = (now-starttime)/(i+1);
				Util.pf("Done with file %d/%d, average time is %.01f millis\n", i, pathlist.size(), avg);
			}
	...
	void processFile(String filepath)
	{
		...
		try { 
			// Util.pf("Running for file path %s\n", filepath);
